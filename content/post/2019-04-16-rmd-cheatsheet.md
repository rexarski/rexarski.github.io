...
1. Blogging with `blogdown` {{< math "inline" >}} \equiv {{< /math >}} writing reports with `knitr`. There is no need to "knit" every post you write, no matter which format (`.md` or `.Rmd`) you are using, since `blogdown` will handle them itself.
